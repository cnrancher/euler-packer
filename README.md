...
1. Install build dependencies and prepare.
    ...
    - Make sure AWS config file `~/.aws/config` is configured (generated by command `aws configure`)and `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` environment variables are configured.
