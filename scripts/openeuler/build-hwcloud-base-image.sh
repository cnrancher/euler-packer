...
if [[ "$1" == "-h" || "$1" == "--help" ]]; then
    ...
    echo "Example: "
    echo "      OPENEULER_VERSION=24.03-LTS-SP3 OPENEULER_ARCH=x86_64 BUCKET_NAME=openeuler-packer BUCKET_LOCATION=ap-southeast-1 $0"
    exit 0
fi
...
# Upload shrinked qcow2 image to bucket
